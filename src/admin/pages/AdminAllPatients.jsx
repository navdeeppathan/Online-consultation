...
const AdminAllPatients = () => {
    ...
    const fetchPatients = async () => {
      ...
      try {
        const response = await axios.get(
          `${Config.BASE_URL}/api/admingetAllPatients`
        );
  ...
  const toggleStatus = async (patient) => {
    setStatusLoading((prev) => [...prev, patient.id]);
    ...
    try {
      ...
      toast.success(
        `Patient ${patient.firstname} is now ${
      ...
      setStatusLoading((prev) => prev.filter((id) => id !== patient.id));
    }
  };
                        ...
                        <tr key={patient.id}>
                          ...
                          <td>
                            <label className="switch">
                              <input
                                type="checkbox"
                                checked={patient.status === 0}
                                onChange={() => toggleStatus(patient)}
                                disabled={statusLoading.includes(patient.id)}
                              />
                              <span className="slider">
                                {statusLoading.includes(patient.id) ? (
                          ...
                          <td>
                            <button
                              ...
                              onClick={() =>
                                navigate(
                                  `/admin-patient-profile?id=${patient.id}`
                                )
                              }
                            >
                              View Details →
