...
const AllDoctorpageinperson = () => {
  ...
  const fetchDrData = async () => {
    try {
      ...
      const inPersonDoctors = allDoctors.filter((doctor) =>
        doctor.availability?.some((slot) =>
          slot.consultation_modes?.includes("in-person")
        )
      );

      console.log("inPersonDoctors", inPersonDoctors);

      setAlldoctor(inPersonDoctors);
      setFilteredDoctors(inPersonDoctors);
    } catch (error) {
      console.error(
...
export default AllDoctorpageinperson;
